"""Gewichteter gerichteter Graph mit Adjazenzlisten.

Ein gewichteter gerichteter Graph, implementiert mit Adjazenzlisten.
Parallele Kanten und Selbstschleifen sind erlaubt.

Ausführung:
    python edge_weighted_digraph.py dateiname.txt

Datendateien:
    https://algs4.cs.princeton.edu/43mst/tinyEWG.txt
    https://algs4.cs.princeton.edu/43mst/mediumEWG.txt
    https://algs4.cs.princeton.edu/43mst/largeEWG.txt

Beispiel:
    >>> python edge_weighted_digraph.py tinyEWD.txt
    8 vertices, 15 edges
    0: 0->2 0.26000 0->4 0.38000
    1: 1->3 0.29000
    2: 2->7 0.34000
    3: 3->6 0.52000
    4: 4->7 0.37000 4->5 0.35000
    5: 5->1 0.32000 5->7 0.28000 5->4 0.35000
    6: 6->4 0.93000 6->0 0.58000 6->2 0.40000
    7: 7->3 0.39000 7->5 0.28000
"""

from src.algs4.pva_1_fundamentals.bag import Bag

from .directed_edge import DirectedEdge


class EdgeWeightedDigraph:
    """Ein gewichteter gerichteter Graph mit Adjazenzlisten.

    Diese Klasse repräsentiert einen gewichteten gerichteten Graphen,
    implementiert mit Adjazenzlisten. Parallele Kanten und Selbstschleifen
    sind erlaubt.

    Attribute:
        V (int): Die Anzahl der Knoten
        E (int): Die Anzahl der Kanten
        adj (dict): Adjazenzlisten (Knoten -> Bag von Kanten)
    """

    def __init__(self, v: int = 0, **kwargs) -> None:
        """Initialisiert einen neuen gewichteten Digraph.

        Args:
            v: Die Anzahl der Knoten
            **kwargs: Optional 'file' für Datei-basierte Initialisierung

        Raises:
            ValueError: Wenn v negativ ist
        """
        if v < 0:
            raise ValueError("Anzahl der Knoten darf nicht negativ sein")

        self.V = v
        self.E = 0
        self.adj = {}
        for i in range(self.V):
            self.adj[i] = Bag()

        if "file" in kwargs:
            # Initialisiere Digraph aus Datei
            in_file = kwargs["file"]
            self.V = int(in_file.readline())
            for i in range(self.V):
                self.adj[i] = Bag()
            num_edges = int(in_file.readline())
            for _ in range(num_edges):
                v, w, weight = in_file.readline().split()
                self.add_edge(DirectedEdge(int(v), int(w), float(weight)))

    def add_edge(self, e: DirectedEdge) -> None:
        """Fügt eine Kante zum Graphen hinzu.

        Args:
            e: Die hinzuzufügende Kante
        """
        self.adj[e.From()].add(e)
        self.E += 1

    def edges(self) -> list:
        """Gibt alle Kanten des Graphen zurück.

        Returns:
            Liste aller Kanten
        """
        edges = []
        for v in range(self.V):
            for e in self.adj[v]:
                edges.append(e)
        return edges

    def __str__(self) -> str:
        """Gibt eine String-Darstellung des Graphen zurück.

        Returns:
            String mit Knoten- und Kantenanzahl sowie Adjazenzlisten
        """
        s = f"{self.V} vertices, {self.E} edges\n"
        for i in range(self.V):
            adjs = " ".join([str(x) for x in self.adj[i]])
            s += f"{i}: {adjs}\n"
        return s

    def __repr__(self) -> str:
        """Gibt eine detaillierte String-Darstellung zurück."""
        return f"EdgeWeightedDigraph({self.V} vertices, {self.E} edges)"


if __name__ == "__main__":
    import sys

    graph = EdgeWeightedDigraph(file=open(sys.argv[1]))
    print(graph)
