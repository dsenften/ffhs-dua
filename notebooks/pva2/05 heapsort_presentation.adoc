...
**Max-Heap**: Jeder Knoten ≥ seine Kinder

== HeapSort Algorithmus

[.step]
. **Heap-Konstruktion**: Array → Max-Heap
. **Sortdown**: Wiederhole:
...
[.step]
* **Tausche 16 ↔ 3**: [3, 4, 10, 2, 1, 9, **16**]
* **Sink 3**: [10, 4, 9, 2, 1, 3, **16**]
* **Tausche 10 ↔ 3**: [3, 4, 9, 2, 1, **10, 16**]
...
```
Heap-Konstruktion:
[4,1,3,2,16,9,10] → [16,4,10,2,1,9,3]

Sortdown:
[16,4,10,2,1,9,3] → [10,4,9,2,1,3,16]
[10,4,9,2,1,3,16] → [9,4,3,2,1,10,16]
[9,4,3,2,1,10,16] → [4,2,3,1,9,10,16]
...
[1,2,3,4,9,10,16]
```
...
* **n mal Sortdown**: n × O(log n)
* **Gesamt**: O(n log n)

**Warum O(n) für Heapify?**
Meiste Knoten sind Blätter!
...
Gesamt ≤ Σ(i=0 bis h) 2^i × (h-i) = O(n)
```
...
|In-Place (O(1) Speicher) |Schlechte Cache-Lokalität
...
```
📁 src/algs4/sorting/heap.py
📁 tests/test_sorting/test_heap.py
```
